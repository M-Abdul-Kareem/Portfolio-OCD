...
db.commit()
# Capture final values BEFORE closing the session, so the summary
# printout below never has to touch the DB again.
final_username = admin.username
final_email = admin.email
db.close()
print("\n✅ Admin account updated.")
print(f"   Username: {final_username}")
print(f"   Email:    {final_email or '(not set)'}")
